...
export async function runStockAnalysis({ input, llmConfig, experienceMode = 'beginner', investorPolicy = null, callLlm: invokeLlm = callLlm, onDelta = null }) {
  ...
  const COMPLIANCE_RULES = '合规铁律：预测价位必须写明计算依据（来自给定数据，不得虚构）；每个判断标注确定性；本分析是交易框架推演而非收益承诺，最终决策权与盈亏责任归用户。';
  const PRICE_TABLE_RULE = [
    '【核心买卖参考价 · 硬性要求】无论结论是买入还是等待，都必须在回答最前面给出「三价表」（Markdown 表格）：',
    '| 项目 | 参考价 | 计算依据 |',
    '| 建议买入触发价 | 具体数字 | 写明算法（如：突破压力位 X 元且量能放大 / 回踩 MA20 企稳 / 站上密集成交区上沿） |',
    '| 止损价 | 具体数字 | 写明算法（如：买入价 × 0.92，或跌破关键支撑 X 元） |',
    '| 目标卖出价 | 第一档数字 / 第二档数字 | 写明算法（如：第一档压力位 +3%，第二档下一压力位，或按 +20%~25% 分批） |',
    '三价必须相互自洽（止损距买入约 -7%~-8%，目标至少是止损空间的 2 倍以上才值得做）；并给一句预期节奏（短线几天内 / 波段几周）。',
    '当前若不是买点，也要给出「若未来触发买入条件时的三价预案」——预测就是你的本职，不许只说「观望」而不给数。',
  ].join('\n');
