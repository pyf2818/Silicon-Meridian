/**
 * 【补丁快照】patch 0010（buildSystemPrompt depth-mode）应用后的完整文件内容。
 * 用途：在补丁实际应用（写工作树被环境锁拦截）之前，让 promptDepth.test.js 能验证补丁后行为。
 * patch 0010 应用后，本文件内容与 buildSystemPrompt.js 一致，可安全删除（连同测试中对本文件的 import）。
 * 生成：批 4 / .workbuddy/gen-patch-b4.mjs
 */
